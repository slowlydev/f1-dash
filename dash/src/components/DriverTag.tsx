...
export default function DriverTag({ position, teamColor, short, className }: Props) {
	return (
		<div
			className={clsx("flex w-fit items-center justify-between gap-0.5 rounded-lg px-1 py-1 font-black bg-gray-700", className)}
		...
		>
			...
			<div className="flex h-min w-min items-center justify-center rounded-md bg-white px-1">
				<p className="text-gray-700" style={{ color: `#${teamColor}` }}>{short}</p>
			</div>
		</div>
	);
}
