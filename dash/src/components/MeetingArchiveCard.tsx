...
export default function MeetingArchiveCard({ meeting }: Props) {
	return (
		<div className="flex flex-col gap-4 py-4">
			...
			<div className={clsx("grid grid-cols-2 gap-2", "sm:grid-cols-3", "md:grid-cols-4", "lg:grid-cols-5")}>
				{meeting.sessions.map((session) => (
					<MeetingArchiveSession key={`${session.number}.${session.type}`} session={session} />
				))}
			</div>
		</div>
	);
