...
export default function SettingsPage() {
	...
	return (
		<div
			className={`min-h-screen w-full ${settings.darkMode ? "bg-background-dark text-white" : "bg-background-light text-black"}`}
		>
			<div className={`container mx-auto max-w-screen-lg px-4`}>
				...
				<div className="flex gap-2">
					<Toggle enabled={settings.darkMode} setEnabled={(v) => settings.setDarkMode(v)} />
					<p className="text-zinc-500">Dark Mode</p>
				</div>
				...
				<p className="mb-4">
					Here you have to option to set a delay for the data, it will displayed the amount entered in seconds later
					than on the live edge. On the Dashboard page there is the same delay input field so you can set it without
					going to the settings. It can be found in the most top bar on the right side.
				</p>
			...
			<div className="flex gap-2">
				{favoriteDrivers.map((driverNumber) => {
					...
					return (
						<div
							key={driverNumber}
							className={`flex items-center gap-1 rounded-xl border p-1 ${darkMode ? "border-secondary-dark" : "border-secondary-light"}`}
						>
