...
const SubLayout = ({ children }: Props) => {
  const { state, setState, setConnected, delay, setDelay, connected } =
    useSocket();
  ...
  const addStateToBuffer = (state: State) => {
    setBuffer((prevBuffer) => {
      const newBuffer = [
        ...prevBuffer,
        { state, timestamp: Date.now(), used: false },
      ];
  ...
  const maxDelay =
    buffer.length > 0
      ? Math.floor((Date.now() - buffer[0].timestamp) / 1000)
      : 0;
          ...
          <div className="absolute z-10 h-full w-full">
            <div className="flex h-full w-full flex-col items-center justify-center backdrop-blur-lg">
              <p className="text-3xl font-medium">
                Syncing, wait for {delay - maxDelay}s
              </p>
